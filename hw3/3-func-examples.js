...
console.log(newData, newDataElse); // "Vasya", 20; "Vasya",
// undefined - во втором случае значение аргумента не передавалось в функцию,
// так что ключу age не было присвоено значение. Значение 'Vasya' берется из переменной name
...
canGoToManGym("Lily", 24, "woman"); // false. 24 > 90 - false, gender woman === man - false. return false && false будет false
canGoToManGym("Vasya", 30, "man"); // false. 30 > 90 - false, gender nam === man - true. return false && true будет false
...
getName("Lily"); // Lily - потому что передали значение аргумента
getName(); // значение не передали поэтому подставится "Default name"
...
function getNumberInRange( rangeStart, rangeEnd, number) {
  ...
  }// если введенные значения будут возвразать true при isNan() то юзера попросят ввести их повторно а потом все окошки для ввода появятся еще раз
...
getNumberInRange(
  ...
  +prompt("Please enter range to"),
  +prompt("Please enter number to check in range")
);// если значение number будет между значениями rangeStart, rangeEnd, - функция возвращает сообщение подставляя аргументы функции в строку
