...
var j = 0;
function howMany(n) {
    // тут бы еще в идеале проверять что n это число =)
...
var arr = ['1', '2', '3', '4', '5', '6'];
// если в массиве не четное количество элементов все немножечко ламаеться(
var newArr = [];
function arrIntoStr() {
    for (i = 0; i < arr.length; i++) {
        var x = arr.shift();
...
var arr = [2, 3, 1, 4];
var newArr = [];
// эту переменную лучше было бы обьявить внутри функции потому что если ты второй раз вызовешь 
// функцию она будет уже не правильно работать
function adder() {
    for (i = 0; i < arr.length; i++) {
        for (j = 0; j < arr[i]; j++) {
            newArr.push(arr[i]);
